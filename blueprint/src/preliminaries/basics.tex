...
\begin{definition}[FreeAlgebra]
    ...
    An \vocab{free algebra} $A$ over $R$ is a triple $(A, +, *)$ constructed inductively by:

    \begin{enumerate}

    \item for all $a in R$, there exists a map $R \to A$
    ...
    \item $A$ is a \vocab{commutative ring} under $+$.
    \item $A$ is a \vocab{monoid} under $*$.
    \item \vocab{associativity} and \vocab{commutativity} under $*$.
    \item \vocab{associativity} under $*$.
    \item $1 * a = a * 1 = a$.
    \item $0 * a = a * 0 = 0$.
    \item left and right \vocab{distributivity} under $*$ over $+$
    \item for all $a, b, c$ in $A$,
